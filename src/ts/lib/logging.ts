...
export class logger {
    ...
    public static async clearConsole() {
        ...
        if (internalConsole) {
            [...internalConsole.getElementsByTagName("span")].forEach(
    ...
    private static async sendToLogs(message: string, type: string) {
        ...
        let currentContents = await fs.readTextFile(
            APPDATA_PATH + "9Launcher.log",
        );
        await fs.writeTextFile(
            "9Launcher.log",
            currentContents +
                `${Date.now()} -> [9L] ${log.type}: ${log.message} \n`,
    ...
    public static async initLogFile() {
        ...
        await logger.info(
            "9Launcher Kernel Version: " + info.kernelVersion,
            true,
            false,
        );
        await logger.info(
            "9Launcher Architecture: " + info.architecture,
            true,
            false,
        );
    }
